...
#
# The current script version is dev20190505
#
# History :
#
# 20190505 - Fred Denis - Initial release
#
# Variables
#
...
printf "\n\033[1;37m%-8s\033[m\n" "DESCRIPTION"         ;
cat << END
        - `basename $0` has to be run with a user who has SSH equivalence on the cell servers (root is default, use -u to use another user)
        - If `basename $0` is executed as root, it uses ibhosts to build the list of the cell servers to connect to
        - If `basename $0` is executed as a non root user, it uses the databasemachine.xml file to build the list of the cell servers to connect to
        - You can also specify a list of cell to analyze using the -c option
END
printf "\n\033[1;37m%-8s\033[m\n" "OPTIONS"             ;
cat << END
        -v      Shows the details of the bad disks (with error or bad status)
        -u      User to connect to the cells (default is $USER)
