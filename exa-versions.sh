login as: svc.tamans.pythora
svc.tamans.pythora@10.80.19.75's password:
Last login: Thu May 23 15:00:19 2019 from 192.168.237.35
+----------------------------------------------------------------------------+
...
|                                  COVIDIEN                                  |
+============================================================================+
| The use of this computer system is restricted to authorized users          |
| for legitimate business purposes.  Unauthorized access or unauthorized     |
| attempted access is strictly prohibited and may be subject to criminal     |
| prosecution.  Anyone using this system expressly consents to such          |
| monitoring and is advised that if such monitoring reveals possible         |
| evidence of criminal activity, system personnel may provide evidence of    |
| such monitoring to law enforcement officials.                              |
+----------------------------------------------------------------------------+

[SVC.TAMANS.PYTHORA@TAMANS-OR124LV ~]$ ssh root@cmx1db01
root@cmx1db01's password:
Last login: Wed May 22 03:50:26 2019 from tamans-or124lv.thcg.net
[root@cmx1db01 ~]# cd pythian/
[root@cmx1db01 pythian]# vi exa-versions.sh
[root@cmx1db01 pythian]# ./exa-versions.sh

                Cluster is a X2-2 Half rack

           -- Database Servers

      cmx1db01            cmx1db02            cmx1db03            cmx1db04
--------------------------------------------------------------------------------
  18.1.7.0.0.180821   18.1.7.0.0.180821   18.1.7.0.0.180821   18.1.7.0.0.180821
--------------------------------------------------------------------------------


           -- Cells

      cmx1cel01           cmx1cel02           cmx1cel03           cmx1cel04           cmx1cel05           cmx1cel06           cmx1cel07
--------------------------------------------------------------------------------------------------------------------------------------------
  18.1.7.0.0.180821   18.1.7.0.0.180821   18.1.7.0.0.180821   18.1.7.0.0.180821   18.1.7.0.0.180821   18.1.7.0.0.180821   18.1.7.0.0.180821
--------------------------------------------------------------------------------------------------------------------------------------------


         -- Infiniband Switches

     cmx1sw-ib1          cmx1sw-ib2          cmx1sw-ib3
------------------------------------------------------------
       2.2.9-3             2.2.9-3             2.2.9-3
------------------------------------------------------------
...
[root@cmx1db01 pythian]# vi exa-versions.sh
[root@cmx1db01 pythian]# dcli -g ~/dbs_group  -l root "imageinfo -ver -status" | sort | awk -F ": " '{if(node==""){node=$1}; if($2 != "") {status=$3; getline                                                                                ; printf ("%s:%s:%s\n",node, $3, status);  node="" ;}}'
cmx1db01:18.1.7.0.0.180821:success
cmx1db02:18.1.7.0.0.180821:success
cmx1db03:18.1.7.0.0.180821:success
cmx1db04:18.1.7.0.0.180821:success
[root@cmx1db01 pythian]# dcli -g ~/dbs_group  -l root "imageinfo -ver -status" | sort | awk -F ": " '{if(node==""){node=$1}; if($2 != "") {status=$3; getline                                                                                ; printf ("%s:^[^CC%s:%s:%s\n",node, $3, status);  node="" ;}}'
[root@cmx1db01 pythian]#
[root@cmx1db01 pythian]#
[root@cmx1db01 pythian]# stty erase
stty: missing argument to `erase'
Try `stty --help' for more information.
[root@cmx1db01 pythian]# dcli -g ~/dbs_group  -l root "imageinfo -ver -status" | sort | awk -F ": " '{if(node==""){node=$1}; if($2 != "") {status=$3; getline                                                                                ; printf ("%s:%s:%s:%s\n","db", wqnode, $3, status);  node="" ;}}'
db::18.1.7.0.0.180821:success
db::18.1.7.0.0.180821:success
db::18.1.7.0.0.180821:success
db::18.1.7.0.0.180821:success
[root@cmx1db01 pythian]# dcli -g ~/dbs_group  -l root "imageinfo -ver -status" | sort | awk -F ": " '{if(node==""){node=$1}; if($2 != "") {status=$3; getline                                                                                ; printf ("%s:%s:%s:%s\n","db", node, $3, status);  node="" ;}}'
db:cmx1db01:18.1.7.0.0.180821:success
db:cmx1db02:18.1.7.0.0.180821:success
db:cmx1db03:18.1.7.0.0.180821:success
db:cmx1db04:18.1.7.0.0.180821:success
[root@cmx1db01 pythian]# vi exa-versions.sh
[root@cmx1db01 pythian]# ./exa-versions.sh
...
[root@cmx1db01 pythian]# vi exa-versions.sh
[root@cmx1db01 pythian]# dcli -g ~/ib_group -l root version | grep -v BIOS | grep "version:" | awk '{print $1, $NF}' | sort
cmx1sw-ib1: 2.2.9-3
cmx1sw-ib2: 2.2.9-3
cmx1sw-ib3: 2.2.9-3
[root@cmx1db01 pythian]# vi exa-versions.sh
...
[root@cmx1db01 pythian]# vi exa-versions.sh
[root@cmx1db01 pythian]# cat exa-versions.sh
#!/bin/bash
  ...
  then
        ...
        | awk -v NB_PER_LINE="$NB_PER_LINE" ' BEGIN \
                ...
                {       if ($0 !~ /^$/)
                        ...
                        {       printf("%s\n\n", "Note : Please investigate the hosts in red as they have a status = failure returned by the imageinfo comman                                                                                d.")       ;
                        }
                }'
...
login as: svc.tamans.pythora
svc.tamans.pythora@10.80.19.75's password:
Last login: Mon May 27 05:14:49 2019 from 192.168.237.35
+----------------------------------------------------------------------------+
|                       xxxxxxxxxxxxx~~~~~xxxxxxxxxxxxx                      |
|                       xxxxxxxxxxxxx~~~~~xxxxxxxxxxxxx                      |
|                       xxxxxxxxxxxxx~~~~~xxxxxxxxxxxxx                      |
...
[SVC.TAMANS.PYTHORA@TAMANS-OR124LV ~]$ ssh root@cmx1db01
root@cmx1db01's password:
Last login: Sat May 25 10:46:27 2019 from tamans-or127lv.thcg.net
[root@cmx1db01 ~]# cd pythian/
[root@cmx1db01 pythian]# vi exa-versions.sh
[root@cmx1db01 pythian]# ./exa-versions.sh
...
Adb:cmx1db01:18.1.7.0.0.180821:successB
...
Acel:cmx1cel01:18.1.7.0.0.180821:successB
...
./exa-versions.sh: line 146: syntax error near unexpected token `\      '
./exa-versions.sh: line 146: `)\        | awk '{print $0}''
[root@cmx1db01 pythian]# vi exa-versions.sh
[root@cmx1db01 pythian]# ./exa-versions.sh
...
cel:cmx1cel01:18.1.7.0.0.180821:success
cel:cmx1cel02:18.1.7.0.0.180821:success
cel:cmx1cel03:18.1.7.0.0.180821:success
cel:cmx1cel04:18.1.7.0.0.180821:success
cel:cmx1cel05:18.1.7.0.0.180821:success
cel:cmx1cel06:18.1.7.0.0.180821:success
cel:cmx1cel07:18.1.7.0.0.180821:success

ib: cmx1sw-ib1: 2.2.9-3
ib: cmx1sw-ib2: 2.2.9-3
ib: cmx1sw-ib3: 2.2.9-3
...
[root@cmx1db01 pythian]# ./exa-versions.sh  -c
...
[root@cmx1db01 pythian]# ./exa-versions.sh -n 4
...
--------------------------------------------------------------------------------
      ...
      cmx1cel01           cmx1cel02           cmx1cel03           cmx1cel04
--------------------------------------------------------------------------------
  18.1.7.0.0.180821   18.1.7.0.0.180821   18.1.7.0.0.180821   18.1.7.0.0.180821
--------------------------------------------------------------------------------


      cmx1cel05           cmx1cel06           cmx1cel07
------------------------------------------------------------
  18.1.7.0.0.180821   18.1.7.0.0.180821   18.1.7.0.0.180821
------------------------------------------------------------
