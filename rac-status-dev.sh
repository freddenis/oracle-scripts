...
ADVM_DEVICES="False"                                                      # How ADVM devices next to ACFS FS
HIDE_DEVICES="False"                                                      # Hide ACFS FS and ADVM devices
...
END
    ...
    printf "\n\033[1;37m%-8s\033[m\n" "SYNOPSIS"            ;
    cat << END
        $0 [-a] [-n] [-d] [-l] [-s] [-t] [-g] [-v] [-o] [-f] [-L] [-r] [-u] [-k] [-K] [-h]
END
...
END
    ...
    cat << END
        ...
        -k        Shows the ADVM devices on the same line as the ACFS FS (handy to remount some FS)
        -K        Do not show the ACFS FS nor the ADVM devices
...
# Options
while getopts "andslLhg:v:o:f:ekKruw:c:tV" OPT; do
    case ${OPT} in
...
${AWK} -v           NODES="$NODES"           \
       ...
       -v    ADVM_DEVICES="$ADVM_DEVICES"    \
       -v    HIDE_DEVICES="$HIDE_DEVICES"    \
'BEGIN\
{ FS = "="                                   ;
        ...
        while (getline) {
            ...
            if ($1 == "STATE_DETAILS")      {       NB++                                    ;       # Number of instances we came through
                if (DB ~ /acfs/)            {       sub("mounted on ", "", $2)              ;
                                                    acfs_mount[DB] = $2                     ;
                                                    if (length($2) > COL_ACFS)
                                                    { COL_ACFS=length($2)                   ;}}
                if (DB ~ /advm/)            {       sub("Volume device ", "", $2)           ;
                                                    sub(" .*$", "", $2)                     ;
                                                    advm_device[DB]=tolower($2)             ;}
                sub("STATE_DETAILS=", "", $0)           ;
        ...
        for (i = 1; i<=y; i++) {
           ...
           if (HIDE_DEVICES == "False") {
             if (acfs_mount[the_name"."the_type]) {
                 printf("  %-"COL_ACFS"s", acfs_mount[the_name"."the_type]) ;
                 if (ADVM_DEVICES == "True") {
                   printf(" - %s", acfs_device[the_name"."the_type]) ;
                 }
             }
             if (advm_device[the_name"."the_type]) {
                 printf("  %s ", advm_device[the_name"."the_type]) ;
             }
           }
           printf("\n")                                                                 ;
        }
